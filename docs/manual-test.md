...
```bash
helm create demo
helm package demo --destination /tmp/helm-drop
...
# Ctrl+C the server
rm -rf /tmp/helm-repo /tmp/helm-drop
```
...
14:23:01  info: HelmRepoLite[0]   Drop folder: /tmp/helm-drop
14:23:01  info: HelmRepoLite[0] Indexed 0 chart packages from /tmp/helm-repo
14:23:01  info: HelmRepoLite[0] Watching drop folder /tmp/helm-drop
14:23:18  info: HelmRepoLite[0] Imported demo-0.1.0.tgz from drop folder
```
...
- **Drop folder import doesn't fire** — `FileSystemWatcher` can be flaky on network shares. Use a local path. `POST /api/resync` forces a re-scan.
